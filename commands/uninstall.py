...
def app(
    ...
    if not re.match(r"^[a-zA-Z]+@[0-9]+\.[0-9]+\.[0-9]+$", lang_ver):
        typer.echo(typer.rich_utils.rich_format_error(click.ClickException("Invalid command format. Expected format: language@version")))
    ...
    if not os.path.exists(bin_lang_path):
        typer.echo(typer.rich_utils.rich_format_error(click.ClickException(f"No {language} folder found in {BIN_DIR}.")))
        raise typer.Exit(code=1)
    ...
    if os.path.exists(lang_ver_path) and os.path.isdir(lang_ver_path):
        try:
            ...
            typer.echo(typer.rich_utils.rich_format_error(click.ClickException(f"Error removing directory: {e}")))
            raise typer.Exit(code=1)
    else:
        typer.echo(typer.rich_utils.rich_format_error(click.ClickException(f"No {language}-{version} folder found in {bin_lang_path}.")))
        raise typer.Exit(code=1)
    ...
    if language not in versions:
        typer.echo(typer.rich_utils.rich_format_error(click.ClickException(f"No installed versions found for language: {language}")))
        raise typer.Exit(code=1)
